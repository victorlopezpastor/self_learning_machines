...
def input_fields(width,N_psi,M):
    ...
    Psi = np.random.randn(N_psi,width) + 1j*np.random.randn(N_psi,width)

    Psi = normalize( Psi )[:,:,np.newaxis]*(np.linspace(0.1,1,M)[np.newaxis,np.newaxis,:])
    
    
    Theta = np.random.randn(width-N_psi,width) 
    
    Theta = normalize( Theta )[:,:,np.newaxis]*(np.ones(M)[np.newaxis,np.newaxis,:])
    
    
    target = np.random.randn(1) + 1j*np.random.randn(1)
    
    target = target*np.ones(M)
...
def XOR_fields(width,N_psi,M,amp_psi):
    ...
    Theta = np.random.randn(width-N_psi,width) + 1j*0.0
    ...
    target = 0.25*target*amp_psi[np.newaxis,:]
